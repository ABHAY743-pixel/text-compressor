...
class Codec {
  ...
  // It converts the tree_string into huffman_tree
  ...
  encode(data) {
    ...
    // Why padding? Because in the next step we are gonna divide the whole code into 8bit sequences, and from there we will take the corresponding characters for ASCII codes formed by each 8bits
    ...
    // we are taking 8 bit each time, converting it into integer, taking it as ASCII code, and concatenating the corresponding CHARACTER into 'encoded_data'
    let encoded_data = "";
    for (let i = 0; i < binary_string.length; ) {
      ...
      encoded_data += String.fromCharCode(curr_num); // 255 is the maximum value that we can have with 8 bits, and fromCHarCode() helps us to get corresponding character for each ASCII Code
    }
  ...
  decode(data) {
    ...
    for (let i = 0; i < encoded_data.length; i++) {
      let curr_num = encoded_data.charCodeAt(i); // while encoding we were taking corresponding character for ASCII CODES -> now we take the unicodes for corresponding characters

      // we stored the ASCII CODE of the current character in 'curr_num' -> now we need the binary format of this code (because that are the code for all our characters present in the given file) -> so we do the following-
      let curr_binary = "";
      for (let j = 7; j >= 0; j--) {
        let foo = curr_num >> j;
    ...
    for (let i = 0; i < binary_string.length; i++) {
      if (binary_string[i] === "1") node = node[1];
      // 1 (in binary_string) represents the right edge (child), so we move right
      else node = node[0]; // 0 represents left child, so move left

      if (typeof node[0] === "string") {
        // when we encounter a leaf node, we just concatenate it with our 'decoded_data', and more importantly- we start traversing again from the root node
        decoded_data += node[0];
...
function onclickChanges2(secMsg) {
  ...
  msg3.innerHTML =
    " File will be automatically downloaded after the processing is done!";
